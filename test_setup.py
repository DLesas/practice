...
def test_datasets():
    ...
    datasets = {
        'Sample Sales (Ex 0)': 'datasets/exercise_00/sample_sales.csv',
        'Customers (Ex 1)': 'datasets/exercise_01/customers.csv', 
        'Products (Ex 1)': 'datasets/exercise_01/products.csv',
        'Transactions (Ex 1)': 'datasets/exercise_01/transactions.csv',
        'Sensor Metadata (Ex 2)': 'datasets/exercise_02/sensor_metadata.csv',
        'Sensor Readings (Ex 2)': 'datasets/exercise_02/sensor_readings.csv',
        'A/B Test Results (Ex 5)': 'datasets/exercise_05/ab_test_results.csv',
        'Interventions (Ex 5)': 'datasets/exercise_05/interventions.csv'
    }
    
    for name, path in datasets.items():
        if os.path.exists(path):
            try:
                if 'Sensor Readings' in name:
...
def test_basic_operations():
    ...
    try:
        ...
        sensor_df = pd.read_csv('datasets/exercise_02/sensor_readings.csv', nrows=1000)
...
def test_memory_usage():
    ...
    try:
        ...
        # Standard loading
        df1 = pd.read_csv('datasets/exercise_00/sample_sales.csv')
        mem1 = df1.memory_usage(deep=True).sum() / 1024**2
        print(f"Standard loading: {mem1:.2f} MB")
        
        # Optimized loading
        df2 = pd.read_csv('datasets/exercise_00/sample_sales.csv', 
